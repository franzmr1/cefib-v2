/**
 * Componente: ParticipanteForm
 * Versión: 1.0
 * Autor: Franz (@franzmr1)
 * Fecha: 2025-11-26
 * Descripción: Formulario para crear/editar participantes
 */
...
export default function ParticipanteForm({ participanteData, onSuccess }: ParticipanteFormProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const [formData, setFormData] = useState({
    nombres: participanteData?.nombres || '',
    apellidos: participanteData?.apellidos || '',
    tipoDocumento: participanteData?.tipoDocumento || 'DNI',
    numeroDocumento: participanteData?.numeroDocumento || '',
    email: participanteData?.email || '',
    telefono: participanteData?.telefono || '',
    celular: participanteData?.celular || '',
    direccion: participanteData?.direccion || '',
    estado: participanteData?.estado || 'ACTIVO',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = participanteData 
        ? `/api/participantes/${participanteData.id}`
        : '/api/participantes';
      
      const method = participanteData ? 'PUT' : 'POST';
      ...
      if (!response.ok) {
        if (data.details) {
          ...
          // Error general
          alert(data.error || 'Error al guardar participante');
        }
        return;
      }

      // Éxito
      alert(data.message || 'Participante guardado exitosamente');
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          ...
          {isSubmitting ? 'Guardando...' : participanteData ? 'Actualizar' : 'Crear Participante'}
        </button>
